...
bool WavesCSApp::Initialize()
{
	...
	mWaves = std::make_unique<Waves>(
		md3dDevice.Get(),
		mCommandList.Get(),
		250, 250, 1.0f, 0.03f, 4.0f, 0.2f);

	LoadTextures();
	BuildRootSignature();
	BuildDescriptorHeaps();
	mWaves->BuildWavesCSResource();

	CD3DX12_CPU_DESCRIPTOR_HANDLE hCpu{ mDescriptorHeap->GetCPUDescriptorHandleForHeapStart() };
	hCpu.Offset(GetNumDescriptors(), mCbvSrvUavDescriptorSize);
	CD3DX12_GPU_DESCRIPTOR_HANDLE hGpu{ mDescriptorHeap->GetGPUDescriptorHandleForHeapStart() };
	hGpu.Offset(GetNumDescriptors(), mCbvSrvUavDescriptorSize);
	mWaves->BuildDescriptorHeaps(hCpu, hGpu, mCbvSrvUavDescriptorSize);
...
void WavesCSApp::BuildRootSignature()
{
	...
	rootParameter[4].InitAsDescriptorTable(1, &displacementMapTable, D3D12_SHADER_VISIBILITY_ALL);
	...
	mWaves->BuildComputeRootSignature();
}

UINT WavesCSApp::GetNumDescriptors()
{
...
void WavesCSApp::BuildDescriptorHeaps()
{
	...
	srvHeap.NumDescriptors = GetNumDescriptors() + mWaves->GetNumDescriptors();
...
void WavesCSApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS_CS"] = d3dUtil::CompileShader(L"Shaders/VertexShader.hlsl", nullptr, "mainCS", "vs_5_0");
...
template<typename T>
void WavesCSApp::MakeGeometry(const std::string&& geoName, const std::string&& smName,
...
D3D12_SHADER_BYTECODE WavesCSApp::GetShaderBytecode(const std::string&& shaderName)
{
	return { mShaders[shaderName]->GetBufferPointer(), mShaders[shaderName]->GetBufferSize() };
}

void WavesCSApp::MakeOpaqueDesc(D3D12_GRAPHICS_PIPELINE_STATE_DESC* inoutDesc)
{
...
void WavesCSApp::MakeAlphaTestedDesc(D3D12_GRAPHICS_PIPELINE_STATE_DESC* inoutDesc)
{
	inoutDesc->PS = GetShaderBytecode("alphaTestedPS");
	inoutDesc->RasterizerState.CullMode = D3D12_CULL_MODE_NONE;
}

void WavesCSApp::MakeTransparentDesc(D3D12_GRAPHICS_PIPELINE_STATE_DESC* inoutDesc)
{
...
void WavesCSApp::MakePSOPipelineState(GraphicsPSO psoType)
{
	...
	case GraphicsPSO::TransparentCS:			MakeTransparentDesc(&psoDesc);
		psoDesc.VS = GetShaderBytecode("standardVS_CS");
		break;
...
void WavesCSApp::UpdateWaves(const GameTimer& gt)
{
...
void WavesCSApp::DrawRenderItems(
...
void WavesCSApp::Draw(const GameTimer& gt)
{
	...
	mCommandList->SetPipelineState(mGraphicsPSOs[GraphicsPSO::TransparentCS].Get());
