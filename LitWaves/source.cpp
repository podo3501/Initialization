...
void LitWavesApp::BuildLandGeometry()
{
	...
	for (auto i : Range(0, (int)grid.Vertices.size()))
	{
...
void LitWavesApp::BuildMaterials()
{
	...
	water->Name = "water";
	water->MatCBIndex = 1;
	water->DiffuseAlbedo = XMFLOAT4(0.0f, 0.2f, 0.6f, 1.0f);	//물에 알파값이 1.0f가 왜 들어가는지 모르겠다.
...
void LitWavesApp::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for (auto& e : mAllRitems)
	{
		...
		currObjectCB->CopyData(e->ObjCBIndex, objectConstants);
		e->NumFramesDirty--;
	}
}
...
void XMStoreFloat4x4Transpose(DirectX::XMFLOAT4X4* dest, XMMATRIX& src)
{
...
void LitWavesApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	XMStoreFloat4x4Transpose(&mainPassCB.View, view);
	XMStoreFloat4x4Transpose(&mainPassCB.Proj, proj);
	XMStoreFloat4x4Transpose(&mainPassCB.ViewProj, viewProj);
	XMStoreFloat4x4Transpose(&mainPassCB.InvView, invView);
	XMStoreFloat4x4Transpose(&mainPassCB.InvProj, invProj);
	XMStoreFloat4x4Transpose(&mainPassCB.InvViewProj, invViewProj);
	...
	//mainPassCB.Lights[0].Strength = { 0.0f, 0.0f, 0.0f };
	/*static float tTime = 0.0f;
	tTime += (gt.DeltaTime() * 10.0f);
	float strengthRed = sin(tTime);
	if (strengthRed < 0.0f) strengthRed *= -1;
	mainPassCB.Lights[0].Strength = { strengthRed, 0.0f, 0.0f };*/
	...
	mainPassCB.InvRenderTargetSize = XMFLOAT2(1.0f / (float)mClientWidth, 1.0f / (float)mClientHeight);
	...
	const std::vector<RenderItem*>& ritems)
{
	...
	for (auto ri : ritems)
	{
		...
		D3D12_GPU_VIRTUAL_ADDRESS objCBAddress = objectCBRes->GetGPUVirtualAddress() +
			ri->ObjCBIndex * objectCBByteSize;
		D3D12_GPU_VIRTUAL_ADDRESS matCBAddress = matCBRes->GetGPUVirtualAddress() +
