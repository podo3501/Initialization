...
void BlurApp::BuildPostProcessRootSignature()
{
	...
	CD3DX12_ROOT_PARAMETER rootParameter[ParamCount];
	rootParameter[0].InitAsConstants(12, 0);
	rootParameter[1].InitAsDescriptorTable(1, &srvTable);
	rootParameter[2].InitAsDescriptorTable(1, &uavTable);

	CreateRootSignature({}, rootParameter, ParamCount, md3dDevice.Get(), mPostProcessRootSignature);
}

void BlurApp::BuildDescriptorHeaps()
{
	UINT numSrvDesc = static_cast<UINT>(mTextures.size());
	UINT numDescriptors = numSrvDesc + mBlurFilter->GetNumDescriptors();
	...
	mBlurFilter->BuildDescriptors(
		CD3DX12_CPU_DESCRIPTOR_HANDLE(mDescriptorHeap->GetCPUDescriptorHandleForHeapStart(), 
			numSrvDesc, mCbvSrvUavDescriptorSize),
...
void BlurApp::BuildShadersAndInputLayout()
{
	...
	mShaders["HBlurCS"] = d3dUtil::CompileShader(L"Shaders/ComputeShader.hlsl", nullptr, "HorzBlurCS", "cs_5_0");
	mShaders["VBlurCS"] = d3dUtil::CompileShader(L"Shaders/ComputeShader.hlsl", nullptr, "VertBlurCS", "cs_5_0");
...
template<typename T>
void BlurApp::MakeGeometry(const std::string&& geoName, const std::string&& smName,
...
D3D12_SHADER_BYTECODE BlurApp::GetShaderBytecode(const std::string&& shaderName)
{
	return { mShaders[shaderName]->GetBufferPointer(), mShaders[shaderName]->GetBufferSize() };
}

void BlurApp::MakeOpaqueDesc(D3D12_GRAPHICS_PIPELINE_STATE_DESC* inoutDesc)
{
...
void BlurApp::MakeAlphaTestedDesc(D3D12_GRAPHICS_PIPELINE_STATE_DESC* inoutDesc)
{
	inoutDesc->PS = GetShaderBytecode("alphaTestedPS");
	inoutDesc->RasterizerState.CullMode = D3D12_CULL_MODE_NONE;
}

void BlurApp::MakeTransparentDesc(D3D12_GRAPHICS_PIPELINE_STATE_DESC* inoutDesc)
{
...
void BlurApp::MakePSOPipelineState(GraphicsPSO psoType)
{
...
void BlurApp::MakePSOPipelineState(ComputePSO psoType)
{
	D3D12_COMPUTE_PIPELINE_STATE_DESC computePsoDesc{};
	computePsoDesc.pRootSignature = mPostProcessRootSignature.Get();
	computePsoDesc.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;

	switch (psoType)
	{
	case ComputePSO::HorzBlur:	computePsoDesc.CS = GetShaderBytecode("HBlurCS");		break;
	case ComputePSO::VertBlur:		computePsoDesc.CS = GetShaderBytecode("VBlurCS");		break;
	default: assert(!"wrong type");
	}
...
void BlurApp::Draw(const GameTimer& gt)
{
	...
	mBlurFilter->Execute(mCommandList.Get(), mPostProcessRootSignature.Get(),
		mComputePSOs[ComputePSO::HorzBlur].Get(), mComputePSOs[ComputePSO::VertBlur].Get(), CurrentBackBuffer(), 4);
